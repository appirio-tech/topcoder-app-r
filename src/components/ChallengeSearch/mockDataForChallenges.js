/*eslint-disable */
const mockChallengeData = [
  {
    "id": 30049141,
    "name": "SWIFT for Puneet dev challenge",
    "description": null,
    "track": "DEVELOP",
    "status": "ACTIVE",
    "createdBy": "23235151",
    "createdAt": 1427937857,
    "updatedBy": "22841596",
    "updatedAt": 1428358931,
    "subtrack": "CODE",
    "submissions": 0,
    "registrants": 0,
    "phases": [
      {
        "duration": 432000000,
        "createdAt": 1427923738,
        "updatedBy": "22841596",
        "actualStartTime": 1427926344,
        "createdBy": "23235151",
        "scheduledStartTime": 1427926344,
        "scheduledEndTime": 1428358492,
        "actualEndTime": 1428358492,
        "id": 732715,
        "type": "REGISTRATION",
        "updatedAt": 1428358972,
        "status": "CLOSED"
      },
      {
        "duration": 431700000,
        "createdAt": 1427923738,
        "updatedBy": "22841596",
        "actualStartTime": 1427927163,
        "createdBy": "23235151",
        "scheduledStartTime": 1427927163,
        "scheduledEndTime": 1428358952,
        "actualEndTime": 1428358952,
        "id": 732716,
        "type": "SUBMISSION",
        "updatedAt": 1428358972,
        "status": "CLOSED"
      }
    ],
    "prizes": [
      {
        "amount": 350,
        "id": 241982,
        "place": 1
      },
      {
        "amount": 150,
        "id": 241983,
        "place": 2
      }
    ],
    "users": [
      {
        "roles": [
          "SUBMITTER"
        ],
        "handle": "albertwang",
        "id": 10336829,
        "placement": null
      },
      {
        "roles": [
          "MANAGER"
        ],
        "handle": "appiriowes",
        "id": 23252679,
        "placement": null
      },
      {
        "roles": [
          "MANAGER"
        ],
        "handle": "Applications",
        "id": 22770213,
        "placement": null
      }
    ]
  },
  {
    "id": 30049480,
    "name": "foo3 design",
    "description": null,
    "track": "DESIGN",
    "status": "ACTIVE",
    "createdBy": "40141235",
    "createdAt": 1447370607,
    "updatedBy": "8547899",
    "updatedAt": 1447778169,
    "subtrack": "WIDGET_OR_MOBILE_SCREEN_DESIGN",
    "submissions": 3,
    "registrants": 6,
    "phases": [
      {
        "duration": 172800000,
        "createdAt": 1447352608,
        "updatedBy": "8547899",
        "actualStartTime": 1447352824,
        "createdBy": "40141235",
        "scheduledStartTime": 1447352824,
        "scheduledEndTime": 1447352881,
        "actualEndTime": 1447352881,
        "id": 733517,
        "type": "SPECIFICATION_SUBMISSION",
        "updatedAt": 1447778169,
        "status": "CLOSED"
      },
      {
        "duration": 1000,
        "createdAt": 1447352608,
        "updatedBy": "8547899",
        "actualStartTime": 1447352883,
        "createdBy": "40141235",
        "scheduledStartTime": 1447352883,
        "scheduledEndTime": 1447371578,
        "actualEndTime": 1447371578,
        "id": 733518,
        "type": "SPECIFICATION_REVIEW",
        "updatedAt": 1447778169,
        "status": "CLOSED"
      }
    ],
    "prizes": [
      {
        "amount": 1250,
        "id": 242640,
        "place": 1
      },
      {
        "amount": 250,
        "id": 242641,
        "place": 2
      }
    ],
    "users": [
      {
        "roles": [
          "SPECIFICATION_SUBMITTER"
        ],
        "handle": "jcopilotdev",
        "id": 40141235,
        "placement": null
      }
    ]
  },
  {
    id: 325212,
    name: 'Interlochen Media - iOS Screen Design',
    track: 'DEVELOP',
    subtrack: 'FIRST_2_FINISH',
    status: 'ACTIVE',
    submissions: 20,
    registrants: 34,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      },
      {
        place: 2,
        amount: 500
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "CLOSED",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730389,
        "type": "ITERATIVE_REVIEW",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1424391420000,
        "duration": 12346
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "CLOSED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "billy",
        "roles": ["MANAGER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": true,
          "placement": 1
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false,
        "placement": 2
      }
    ]
  },
  {
    id: 325213,
    name: 'Interlochen Media - iOS Screen Design challenge for iPad - interactive design and 3D visualization',
    track: 'DESIGN',
    subtrack: 'WEB_DESIGNS',
    status: 'ACTIVE',
    submissions: 0,
    registrants: 3,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "OPEN",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "polynickglot",
        "roles": ["SUBMITTER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": false
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false
      }
    ]
  },
  {
    id: 325215,
    name: 'Interlochen Media - iOS Screen Design challenge for iPad - interactive design and 3D visualization enabled through multy-touch and concept - part 45',
    track: 'DATA_SCIENCE',
    subtrack: 'SRM',
    status: 'ACTIVE',
    submissions: 0,
    registrants: 0,
    createdAt: 1424304623000,
    prizes: [
      {
        taskpay: true
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "OPEN",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      }
    ],
    users: []
  },
  {
    id: 325712,
    name: 'Interlochen Media - iOS Screen Design',
    track: 'DEVELOP',
    subtrack: 'FIRST_2_FINISH',
    status: 'ACTIVE',
    submissions: 20,
    registrants: 34,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      },
      {
        place: 2,
        amount: 500
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "CLOSED",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730389,
        "type": "ITERATIVE_REVIEW",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1424391420000,
        "duration": 12346
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "CLOSED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "billy",
        "roles": ["MANAGER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": true,
          "placement": 1
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false,
        "placement": 2
      }
    ]
  },
  {
    id: 325112,
    name: 'Interlochen Media - iOS Screen Design',
    track: 'DEVELOP',
    subtrack: 'FIRST_2_FINISH',
    status: 'ACTIVE',
    submissions: 20,
    registrants: 34,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      },
      {
        place: 2,
        amount: 500
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "CLOSED",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730389,
        "type": "ITERATIVE_REVIEW",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1424391420000,
        "duration": 12346
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "CLOSED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "billy",
        "roles": ["MANAGER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": true,
          "placement": 1
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false,
        "placement": 2
      }
    ]
  },
  {
    id: 125212,
    name: 'Interlochen Media - iOS Screen Design',
    track: 'DEVELOP',
    subtrack: 'FIRST_2_FINISH',
    status: 'ACTIVE',
    submissions: 20,
    registrants: 34,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      },
      {
        place: 2,
        amount: 500
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "CLOSED",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730389,
        "type": "ITERATIVE_REVIEW",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1424391420000,
        "duration": 12346
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "CLOSED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "billy",
        "roles": ["MANAGER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": true,
          "placement": 1
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false,
        "placement": 2
      }
    ]
  },
  {
    id: 32512,
    name: 'Interlochen Media - iOS Screen Design',
    track: 'DEVELOP',
    subtrack: 'FIRST_2_FINISH',
    status: 'ACTIVE',
    submissions: 20,
    registrants: 34,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      },
      {
        place: 2,
        amount: 500
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "CLOSED",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730389,
        "type": "ITERATIVE_REVIEW",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1424391420000,
        "duration": 12346
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "CLOSED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "billy",
        "roles": ["MANAGER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": true,
          "placement": 1
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false,
        "placement": 2
      }
    ]
  },
  {
    id: 3252,
    name: 'Interlochen Media - iOS Screen Design',
    track: 'DEVELOP',
    subtrack: 'FIRST_2_FINISH',
    status: 'ACTIVE',
    submissions: 20,
    registrants: 34,
    createdAt: 1424304623000,
    prizes: [
      {
        place: 1,
        amount: 1000
      },
      {
        place: 2,
        amount: 500
      }
    ],
    phases: [
      {
        "id": 730388,
        "type": "REGISTRATION",
        "status": "CLOSED",
        "startAt": 1424304713484,
        "endAt": 1427302072533,
        "duration": 2997359049
      },
      {
        "id": 730389,
        "type": "ITERATIVE_REVIEW",
        "status": "SCHEDULED",
        "startAt": 1424305073181,
        "endAt": 1424391420000,
        "duration": 12346
      },
      {
        "id": 730390,
        "type": "SUBMISSION",
        "status": "CLOSED",
        "startAt": 1424305073181,
        "endAt": 1427302143442,
        "duration": 12346
      }
    ],
    users: [
      {
        "id": 262467,
        "handle": "billy",
        "roles": ["MANAGER"],
        "winner": false
      },
      {
        "id": 22687838,
        "handle": "andrew.abbott",
        "roles": ["OBSERVER"],
        "winner": false
      },
      {
        "id": 151743,
        "handle": "Ghostar",
        "roles": ["APPROVER", "COPILOT", "ITERATIVE_REVIEWER"],
        "winner": false
      },
      {
        "id": 22908409,
        "handle": "amanforindia",
        "roles": ["SUBMITTER"],
        "winner": true,
          "placement": 1
      },
      {
        "id": 23152282,
        "handle": "sunbinbrother",
        "roles": ["SUBMITTER"],
        "winner": false,
        "placement": 2
      }
    ]
  }
]

export default mockChallengeData
